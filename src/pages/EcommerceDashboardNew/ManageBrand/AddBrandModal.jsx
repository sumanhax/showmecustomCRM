...
const AddHatModal = ({ openModal, setOpenModal, onHatAdded, brandData, isEdit, supplierListData }) => {
  ...
  // Pre-fill form when brandData is provided (for edit mode)
  console.log("brandData",brandData)
  useEffect(() => {
    if (brandData && isEdit && openModal) {
      setValue("name", brandData.name || "");
      setValue("code", brandData.code || "");
      setValue("website_url", brandData.website_url || "");
      setValue(
        "is_active",
        brandData.is_active === 1 || brandData.is_active === true
      );

      // Set image preview if exists
      if (brandData.image_url) {
        setImagePreview(brandData.image_url);
      }
  ...
  }, [brandData, isEdit, openModal, setValue, reset]);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      } else if (isEdit && brandData?.image_url && !selectedImage) {
        // In edit mode, if no new image selected, keep existing image URL
        formData.append("image", brandData.image_url);
      }
      ...
      if (isEdit && brandData) {
        ...
        if (selectedImage) {
          ...
          promises.push(
            dispatch(
              hatEditImageUpload({ id: brandData.id, userInput: imageFormData })
            ).unwrap()
          );
        }
        ...
        promises.push(
          dispatch(hatEdit({ id: brandData.id, userInput: editFormData })).unwrap()
        );
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="flex items-center">
            <input
              ...
              {...register("is_active")}
              defaultChecked={
                brandData?.is_active === 1 || brandData?.is_active === true || true
              }
