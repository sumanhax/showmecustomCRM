...
import { inventoryWiseUpdate } from "../../../Reducer/EcommerceNewSlice";
...
const AddVariantSizeInventoryModal = ({
  ...
  // isEdit হলে form এ পুরানো data prefill করো
  useEffect(() => {
    if (isEdit && inventoryData) {
      setValue("qty_on_hand", inventoryData?.qty_on_hand || "");
      setValue("source", inventoryData?.source || "");
    } else {
      reset();
    }
  }, [isEdit, inventoryData, setValue, reset]);

  const onSubmit = (data) => {
    if (isEdit) {
      // Edit mode — inventoryWiseUpdate thunk (newApi)
      dispatch(
        inventoryWiseUpdate({
          hatId: variantSizeId,
          userInput: {
            qtyOnHand: Number(data.qty_on_hand),
            source: data.source,
            isActive: inventoryData?.is_active ?? 1,
          },
        })
      )
        .unwrap()
      ...
      // Add mode — existing addInventory thunk (পুরানো code হুবহু same)
      dispatch(
        addInventory({ ...data, hat_size_variant_id: variantSizeId })
      )
        .then((res) => {
