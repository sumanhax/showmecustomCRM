...
const AddProjectModal = ({
  ...
  // Watch fields for live calculations and logic
  const watchedProjectType = watch('projectType');
  const watchedOrderAmount = watch('orderAmount');
  const watchedExpense = watch('expense');

  // Calculate Revenue (Order Amount - Expense)
  const calculateRevenue = () => {
    const amount = parseFloat(watchedOrderAmount) || 0;
    const exp = parseFloat(watchedExpense) || 0;
    return (amount - exp).toFixed(2);
  };
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            {/* Order Amount and Expense (Grid Layout) */}
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 border border-gray-200 rounded-md bg-gray-100 text-gray-600 cursor-not-allowed font-medium"
              />
