...
const UpdatePlanKeyModal = ({
  openUpdatePlanKeyModal,
  setOpenUpdatePlanKeyModal,
  planKeyId,
}) => {
  ...
  useEffect(() => {
    dispatch(planKeyList({ plan_key_id: planKeyId })).then((res) => {
      ...
      if (res?.paylod?.status_code === 200) {
        ...
        setValue(
          "plan_extrakey_key",
          res?.payload?.results?.[0]?.plan_extrakey_key
        );
      }
    });
  ...
  return (
    <>
      <Modal
        show={openUpdatePlanKeyModal}
        onClose={() => setOpenUpdatePlanKeyModal(false)}
      >
          ...
          <Modal.Body>
            <div className="space-y-4 max-w-full md:max-w-lg mx-auto px-2 sm:px-4">
              <div>
                ...
                <TextInput
                  id="plan_price_key"
                  {...register("plan_price_key", {
                    required: "Plan Price Key is required",
                  })}
                />
                {errors?.plan_price_key && (
                  <p className="text-red-500 text-sm">
                    {errors.plan_price_key.message}
                  </p>
                )}
              </div>
              <div>
                ...
                <TextInput
                  ...
                  {...register("plan_extrakey_key", {
                    required: "Plan ExtraKey Key is required",
                  })}
                />
                {errors?.plan_extrakey_key && (
                  <p className="text-red-500 text-sm">
                    {errors.plan_extrakey_key.message}
                  </p>
                )}
              </div>
            </div>
